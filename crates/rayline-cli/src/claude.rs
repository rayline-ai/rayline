...
async fn run_command_from_home(
    ...
    // Config-driven routing: an explicit `--router-config-path`, or the default
    // `~/.config/rayline/router.json` (auto-created on first cloud launch), drives
    // routing without `--local`. Engage the on-device router only when the config
    // routes something away from the hosted cloud router; a pure-cloud config (the
    // default) stays on today's hosted path with no local process. `--local`
    // handles its own config separately.
    let effective_config = if request.local_router {
        None
    } else if let Some(flag) = request.router_config_path.as_deref() {
        Some(flag.to_path_buf())
    } else {
        crate::router_config::ensure_default_config(home).ok()
    };
    // `--via env` (Override) is cloud-only by contract; never engage the on-device
    // router from a config under it. An explicit `--router-config-path` + `--via env`
    // is already rejected at parse time, so this only suppresses the default file.
    ...
    let local_start_request = if config_engages_local {
        ...
        // from the effective config. Route-all + main pinned to `rayline-router`
        // (downstream) so `routes.main` governs the main thread.
        ...
        start_request.injector_port = injector_port;
        start_request.router_config_path = Some(path.clone());
        ...
        // effort: if not logged in, the daemon surfaces a clear error only if a
        // cloud-router route is actually exercised.
        ...
        // A `"local"` route needs the bundled model; everything else is config-only
        // (named endpoints handled by the local router directly — no bundled model).
        if crate::router_config::config_uses_local_endpoint(&path) {
            let cfg = local_cfg.as_ref().filter(|cfg| cfg.is_engageable()).ok_or_else(|| {
                RunError::Router(format!(
                    "Router config routes to \"local\", but no local model is configured. Run `{cli} local use <model-id>` or `{cli} local custom ...`, or point that route at a named endpoint.",
                    cli = crate::CLI_BIN,
                ))
            })?;
    ...
    let request = &RunRequest {
        // Config-driven routing must route ALL traffic through the on-device router
        // so `routes.main` governs the main thread (selective-subagents would pass
        // main straight through). Otherwise use the normal local-engagement default.
    ...
    // Config-driven routing pins the main thread to the virtual model so the
    // local router's `routes.main` governs it (a concrete model would be caught by
    // direct-model routing instead). An explicit `--model` still wins.
    let (model, set_model_env) = if config_engages_local && request.model.is_none() {
